...
DOWNLOAD_RUNNER_SOURCE="$RELEASE_ROOT/scripts/run_zsxq_download_pipeline.py"
...
write_deployment_env() {
  local task_dir pipeline source_name canonical_name output_path
  task_dir="$1"
  pipeline="$2"
  source_name="$3"
  canonical_name="$4"
  output_path="$task_dir/deployment.env"
  "$DEPLOY_PYTHON_BIN" - "$output_path" "$RELEASE_ROOT" "$pipeline" "$source_name" "$canonical_name" <<'PY'
from pathlib import Path
import os
import shlex
import sys

output_path, release_root, pipeline, source_name, canonical_name = sys.argv[1:]
...
body = "\n".join(
    [
        ...
        f"DOWNLOAD_RUNNER_PATH={shlex.quote(str(Path(release_root) / 'scripts/run_zsxq_download_pipeline.py'))}",
        f"ZSXQ_SOURCE_NAME={shlex.quote(source_name)}",
        f"NOTIFICATION_PIPELINE={shlex.quote(pipeline)}",
        "NOTIFICATION_POLICY_PATH=\"${AUTOMATION_ROOT}/scripts/zsxq_notification_policy.py\"",
        "PIPELINE_RESULT_PATH=\"${INVESTMENT_REPORTS_RUNTIME_DIR:-${AUTOMATION_ROOT}/.runtime}/logs/" + canonical_name + "\"",
        "",
    ]
)
...
write_deployment_env "$FOREIGN_TASK_DIR" "foreign_download" "foreign_reports" "zsxq_last_run_structured.json"
write_deployment_env "$DOMESTIC_TASK_DIR" "domestic_cicc" "domestic_cicc" "zsxq_domestic_cicc_last_run_structured.json"
