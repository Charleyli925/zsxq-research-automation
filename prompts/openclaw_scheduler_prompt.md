This is the short entry prompt used by `scripts/run_zsxq_task_via_codex.sh`.
It points Codex to the full task prompt below, so this file stays short and stable.
...
- Output machine-readable lines in plain text only.
  ...
  The final result line must start with `ZSXQ_REPORT_JSON:` followed immediately by JSON.
  Example JSON payload after the prefix:
  {"window_new_docs_count":1,"keyword_matched_docs_count":0,"download_candidate_count":0,"download_success_count":0,"no_download_reason":"no_keyword_match","core_reason":"window_has_updates_but_no_keyword_match"}
- Always return:
  1) archived folder path
  2) downloaded filenames
  3) skipped filenames
  4) blocked reasons (if any)
- If login is invalid/expired, do not fake success.
