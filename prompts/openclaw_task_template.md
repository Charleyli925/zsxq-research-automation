...
This is the detailed browser-task prompt.
`openclaw_scheduler_prompt.md` points here, and Codex follows this file during each run.

Use Playwright MCP browser tools to control the dedicated Chrome for Testing instance (fixed executable + persistent profile), not the user's daily Chrome.

## Read these files first

- `${AUTOMATION_ROOT}/config/local/zsxq_foreign_reports_job.json`
- `${AUTOMATION_ROOT}/config/local/interest_keywords.json`
- `${INVESTMENT_REPORTS_RUNTIME_DIR}/state/zsxq_focus_runtime_state.json`
- `${AUTOMATION_ROOT}/prompts/openclaw_runtime_prompt.md`
- `${INVESTMENT_REPORTS_RUNTIME_DIR}/state/zsxq_foreign_reports_state.json`

Important:
- `config/local/interest_keywords.json` is the real keyword source.
- `${INVESTMENT_REPORTS_RUNTIME_DIR}/state/zsxq_focus_runtime_state.json` stores temporary overrides.
- `prompts/openclaw_runtime_prompt.md` is only a generated snapshot for the current run.

## Task

1. Use the launcher-provided `run_id`, `window_start`, and `window_end`. The launcher freezes them after acquiring the shared lock; do not recalculate them from the clock or state file.
...
```bash
python3 scripts/scan_zsxq_download_candidates.py --window-start "<window_start>" --window-end "<window_end>" --output "<scan_plan_path>"
```
...
5. Fast-path rule:
   ...
   - if `download_candidate_count > 0`, immediately start downloading; do not pause to “decide whether to接管下载”
6. DOM fallback rule:
   - only if the scan script reports API failure (`blocked_reason` starts with `api_`) may you consider DOM fallback
   - before DOM fallback, run `python3 scripts/discover_zsxq_topics_api.py` once
   - only if that probe still cannot recover API-first, continue with DOM fallback
7. For each candidate topic (not each file), enter detail once and batch-process all matching PDF attachments in that topic:
   ...
   - keep one browser tool call limited to one file download; do not wrap multiple downloads into one `browser_run_code` call
   ...
   - if return flow is unstable, use once-per-topic recovery:
     `查看原主题 -> 返回 前沿信息收录 -> 外资研报`
8. After each completed browser-download micro-batch, run the same run-aware finalize command. It may be called more than once; each call appends to one aggregate manifest and must not advance the checkpoint:

```bash
python3 scripts/finalize_download_batch.py --window-start "<window_start>" --window-end "<window_end>" --downloaded-after "<run_started_at>" --run-id "<run_id>" --scan-plan "<scan_plan_path>" --run-manifest "<run_manifest_path>" --wait-seconds 0 --skip-state-update
```

Only the launcher may make the final `--commit-state` call after it confirms every planned candidate is reconciled. Never add `--commit-state` from inside the browser task.
...
- When DOM fallback is triggered, run `discover_zsxq_topics_api.py` once to auto-probe a fresh topics API endpoint before deep DOM scan.
- Do not enter topic detail for topics that already fail time/keyword filtering on list/API data.
- Skip duplicates already present under the configured archive root.
